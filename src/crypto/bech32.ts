 ...
 */
function toChars(data: number[]): string {
  let result = '';
 ...
 */
function fromChars(str: string): number[] {
  const result: number[] = [];
  for (let i = 0; i < str.length; i++) {
    const index = CHARSET.indexOf(str.charAt(i));
    if (index === -1) throw new Error(`Invalid character: ${str.charAt(i)}`);
    result.push(index);
  }
  return result;
}
...
export function bech32Decode(str: string): { hrp: string; data: number[] } {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (str.charCodeAt(i) >= 97 && str.charCodeAt(i) <= 122) lower = true;
    if (str.charCodeAt(i) >= 65 && str.charCodeAt(i) <= 90) upper = true;
  }
  if (lower && upper) {
