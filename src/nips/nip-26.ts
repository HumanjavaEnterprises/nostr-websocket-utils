 ...
 */
export function serializeConditions(conditions: DelegationConditions): string {
  const parts: string[] = [];
 ...
 */
export function parseConditions(conditionsString: string): DelegationConditions {
  const conditions: DelegationConditions = {};
  if (!conditionsString) return conditions;
...
function delegationDigest(delegateePubkey: string, conditionsString: string): Uint8Array {
